        ...
        <div class="sidebar-nav-wrapper">
            ...
            <nav class="sidebar-main-nav mt-2">
                <ul class="nav nav-pills nav-sidebar flex-column nav-child-indent" data-widget="treeview" role="menu"
                    ...
                    <x-admin.sidebar-list :linkActive="Route::is('admin.dashboard') ? true : false" route="admin.dashboard" parameter=""  path="admin.dashboard" plus_icon=""
                    ...
                    @if (userCan('company.view'))
                        <x-admin.sidebar-list :linkActive="Request::is('admin/company*') ? true : false" route="company.index"  path="company.create" plus_icon="fa fa-plus-circle"
                    ...
                    @if (userCan('candidate.view'))
                        <x-admin.sidebar-list :linkActive="Route::is('candidate*') ? true : false" route="candidate.index"  path="candidate.create" plus_icon="fa fa-plus-circle"  icon="fas fa-user">
                            {{ __('candidate') }}
                        </x-admin.sidebar-list>
                    @endif
                    <!-- ======== Training_institutes ====== -->
                    {{-- @if (userCan('training_institutes.view'))
                        <x-admin.sidebar-list :linkActive="Route::is('candidate*') ? true : false" route="candidate.index"  path="candidate.create" plus_icon="fa fa-plus-circle"  icon="fas fa-user">
                            {{ __('training_institutes') }}
                        </x-admin.sidebar-list>
                    @endif --}}
                    <x-admin.sidebar-list :linkActive="Route::is('training_institute*') ? true : false" route="training_institute.index" path="training_institute.create" plus_icon="fa fa-plus-circle" icon="fas fa-user">
                    {{ __('training_institutes') }}
                    </x-admin.sidebar-list>
                    ...
                    @if (userCan('job_category.view'))
                        <x-admin.sidebar-list :linkActive="Request::is('admin/jobCategory*') ? true : false" route="jobCategory.index"  path="jobCategory.index" plus_icon=""  icon="fas fa-th">
                            {{ __('job_category') }}
                        </x-admin.sidebar-list>
                    @endif
                    <!-- ======= Job Role ======== -->
                    @if (userCan('job_role.view'))
                        <x-admin.sidebar-list :linkActive="Request::is('admin/jobRole*') ? true : false" route="jobRole.index"  path="jobRole.index" plus_icon=""  icon="fas fa-user-tie">
                            {{ __('job_role') }}
                        </x-admin.sidebar-list>
                    @endif
                        ...
                        Request::is('admin/organizationType*')
                            ...
                            <ul class="nav nav-treeview">
                                ...
                                @if (userCan('skills.view'))
                                    <x-admin.sidebar-list :linkActive="request()->routeIs('skill.*') ? true : false" route="skill.index"  path="skill.index" plus_icon=""  icon="fas fa-cog">
                                        {{ __('skills') }}
                                    </x-admin.sidebar-list>
                                @endif
                                <!-- ======= tags ======== -->
                                @if (userCan('tags.view'))
                                    <x-admin.sidebar-list :linkActive="request()->routeIs('tags.*') ? true : false" route="tags.index"  path="tags.index" plus_icon=""  icon="fas fa-tags">
                                        {{ __('tags') }}
                                    </x-admin.sidebar-list>
                                @endif
                                <!-- ======= benefits ======== -->
                    ...
                    @if (userCan('admin.view'))
                        <x-admin.sidebar-list :linkActive="Route::is('user.*') ? true : false" route="user.index"  path="user.create" plus_icon="fa fa-plus-circle"  parameter="" icon="fas fa-users">
                            {{ __('user_role_manage') }}
                        </x-admin.sidebar-list>
                    @endif
                </ul>
            ...
            <nav class="mt-2 nav-footer pt-3 border-top border-secondary">
                <ul class="nav nav-pills nav-sidebar flex-column nav-child-indent" role="menu"
                    ...
                    @if (userCan('setting.view'))
                        <x-admin.sidebar-list :linkActive="request()->is('admin/settings/*') ? true : false" route="settings.general"  path="settings.general"  plus_icon=""  icon="fas fa-cog">
                            {{ __('settings') }}
                        </x-admin.sidebar-list>
                    @endif
                    <li class="nav-item">
